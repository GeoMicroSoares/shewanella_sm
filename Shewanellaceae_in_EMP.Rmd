...
## Filtering the EMP dataset for Shewanellaceae OTUs

From files in Dropbox folder "EMP Paper 01 Supplements":

Table S1_79199_0_data_set_1474767_nx5v7y.xls --> renamed 'EMP_LV.metadata' - **Metadata table**  
Table S2_79199_0_data_set_1474789_nx5wfh.txt --> renamed 'EMP_LV.shared' - **OTU table**  
Table S3_79199_0_data_set_1474790_nx6289.txt --> renamed 'EMP_LV.database' - **Taxonomical table**

# **Step 1**:
  Using 'grep' from the Linux CLI, the Taxonomical table was filtered for any **Shewanellaceae hits**.
```{r, eval=FALSE, include=TRUE}
grep "Shewanella" EMP_LV.database > Shwnl_OTUs.txt
#Got 16 hits - OTUs
```
  File Shwnl_OTUs.txt now contains **.fasta sequence and RDP, SILVA and Greengenes ID's for all Shewanellaceae OTUs in the EMP dataset**.
...
# **Step 2**: 
  Filter the EMP OTU table for the OTU IDs found with **Step 1**:
...
library(dplyr)
library(picante)
EMP <- read.table("EMP_LV.shared", header = TRUE, sep='\t', check.names=FALSE)
dim(EMP)
...
Shwnl_inEMP <- inner_join(Shwnl_OTU_list, EMP, by="OTU")

#Table still huge, eliminate samples with 0 sequences overall:

#turns 1st column into rownames
clean_Shwnl_inEMP <- Shwnl_inEMP[,-1]
rownames(clean_Shwnl_inEMP)<-Shwnl_inEMP[,1]

#any column summing 0 is eliminated
Shwnl_inEMP_noSing <- clean_Shwnl_inEMP[,colSums(clean_Shwnl_inEMP) > 0]
...
# **Step 3**:
  ...
  Importing EMP metadata:
```{r, eval=FALSE}
Shwnl_metadata <- read.table("EMP_LV.metadata", header = TRUE, sep=',', check.names=FALSE)
#937 samples described by 16 E.V.
```

  Extracting Shewanellaceae sample list from the Shewanellaceae-specific OTU table:
```{r, eval=FALSE}
Spl_list_Shwnl_inEMP<-as.data.frame(colnames(Shwnl_inEMP_noSing))
colnames(Spl_list_Shwnl_inEMP)[1] <- "SampleID"
#245 samples
```

  Filtering metadata for samples containing Shewanellaceae OTUs:
```{r, eval=FALSE}
metadata_Shwnl_inEMP <- left_join(Spl_list_Shwnl_inEMP, Shwnl_metadata, by="SampleID")
#245 samples
```
  Transposing OTU table for grouped statistics:
```{r, eval=FALSE}
t_Shwnl_inEMP_noSing <- t(Shwnl_inEMP_noSing)
t_Shwnl_inEMP_noSing <- data.frame(t_Shwnl_inEMP_noSing[,-1], row.names=t_Shwnl_inEMP_noSing[,1])
```

Numbers of OTUs per sponge species (column SCIE_NAME) and country:
```{r, eval=FALSE}
library(vegan)
Shwnl_OTUs_per_SpongeSpecies<-specnumber(t_Shwnl_inEMP_noSing, metadata_Shwnl_inEMP$SCIE_NAME)
Shwnl_OTUs_per_Country<-specnumber(t_Shwnl_inEMP_noSing, metadata_Shwnl_inEMP$COUNTRY)
```

Numbers of sequences per sponge species and country:
```{r, eval=FALSE}
Shwnl_Seqs_per_SpongeSpecies<-aggregate(t_Shwnl_inEMP_noSing, by=list(metadata_Shwnl_inEMP$SCIE_NAME), FUN=sum)
Shwnl_Seqs_per_Country<-aggregate(t_Shwnl_inEMP_noSing, by=list(metadata_Shwnl_inEMP$COUNTRY), FUN=sum)
```

# **Step 4**:
  Getting total OTU and sequence numbers from EMP metadata table, as those are provided from the start.
...
EMP_Country_OTUs_Seqs<-aggregate(. ~ COUNTRY, data=Country, FUN=sum)
Sponges<-as.data.frame(clean_metadata[,c(2,4,5)])
EMP_Sponges_OTUs_Seqs<-aggregate(. ~ SCIE_NAME, data=Sponges, FUN=sum)
...
EMP_FiltbyShwnlCountry_OTUs_Seqs <- inner_join(country_shwnl_list, EMP_Country_OTUs_Seqs, by="COUNTRY")

sponge_shwnl_list<-as.data.frame(Shwnl_Seqs_per_SpongeSpecies$Group.1)
colnames(sponge_shwnl_list)[1]<-"SCIE_NAME"
EMP_FiltbyShwnlSponges_OTUs_Seqs <- inner_join(sponge_shwnl_list, EMP_Sponges_OTUs_Seqs, by="SCIE_NAME")
...
write.table(EMP_FiltbyShwnlSponges_OTUs_Seqs, "EMP_FiltbyShwnlSponges_OTUs_Seqs.csv", sep=",", quote=FALSE)
write.table(EMP_FiltbyShwnlCountry_OTUs_Seqs, "EMP_FiltbyShwnlCountry_OTUs_Seqs.csv", sep=",", quote=FALSE)
```
